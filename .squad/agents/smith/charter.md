...
Before any Epic/Feature or sprint plan is finalized into a GitHub issue or Project item, Smith's draft must be reviewed by **Yoda** (Agile/Scrum methodology advisor — `.squad/agents/yoda/charter.md`) for structural alignment: correct Epic/Feature (single, unified level) vs. Work Item vs. Sprint Task leveling, INVEST-compliant work items, testable acceptance criteria, and correctly sequenced dependencies. On Yoda's ⚠️ or 🔴 verdict, Smith revises based on the specific feedback before the item proceeds to `gh issue create`/Project board.
